...
import { analyzeCompany, type AnalysisOutput } from "./analyzeWithClaude";
import type {
  Executive,
  FilingRef,
  Financials,
...
export async function buildLiveReport(query: string): Promise<string> {
  ...
  // Issue 7: filing date + staleness signal
  const filingDate = ref?.date ?? null;
  const filingDateDisplay = filingDate
    ? new Date(filingDate).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })
    : null;
  const isStale = filingDate
    ? Date.now() - new Date(filingDate).getTime() > 365 * 24 * 60 * 60 * 1000
    : false;
  const stalenessBlock = filingDateDisplay
    ? isStale
      ? `> ⚠️ **Stale data:** This report is based on a 10-K filed **${filingDateDisplay}**, which is over 12 months ago. Confirm key figures before outreach.\n`
      : `> 📋 **Source:** 10-K filed ${filingDateDisplay}. Data reflects that reporting period.\n`
    : '';

  // Issue 8: optional Claude partnership analysis (only when ANTHROPIC_API_KEY is set)
  const yrs = lastN(financials?.revenue ?? [], 3);
  const analysisInput = {
    ticker: hit?.ticker ?? '',
    companyName: name,
    filingDate,
    businessSummary: (tenk?.business ?? '').slice(0, 1000),
    topRisks: (tenk?.risks ?? '').slice(0, 800),
    financialSummary: {
      revenue:   yrs.map(y => y.val),
      netIncome: yrs.map(y => valueFor(financials?.netIncome ?? [], y.fy) ?? null),
      rAndD:     yrs.map(y => valueFor(financials?.rnd ?? [], y.fy) ?? null),
      years:     yrs.map(y => String(y.fy)),
    },
    researchCount: research?.count ?? 0,
  };
  const analysis = await analyzeCompany(analysisInput);
  ...
  const parts: string[] = [
    ...
    sourceManifest(tenk, wiki, financials, research),
...
function companyOverview(
  ...
  // Prefer 10-K Item 1 over Wikipedia — it is primary-source, current, and signed
  // under SEC disclosure obligations. Wikipedia is a fallback for private companies
  // or when the 10-K extraction yields too little.
  const tenKDesc = tenk?.business ? businessLede(tenk.business, name, 600) : '';
  const useWiki = !tenKDesc || tenKDesc.length < 150;
  if (!useWiki) {
    lines.push(tenKDesc + " [4]");
    lines.push(`\n*(Source: SEC 10-K Item 1 — Business)*`);
  } else if (wiki?.extract) {
    lines.push(wiki.extract + " [2]");
    lines.push(`\n*(Source: Wikipedia)*`);
  }
  const facts: string[] = [];
  if (profile?.hqCity)
...
function analysisSection(analysis: AnalysisOutput | null): string {
  if (!analysis) return "";
  return [
    "## Partnership Intelligence Analysis\n",
    `**Partnership Fit**\n${analysis.partnershipFit}`,
    `\n**R&D Trajectory**\n${analysis.rdTrajectory}`,
    `\n**Risk Relevance for Academic Collaboration**\n${analysis.riskRelevance}`,
    `\n**Research Overlap Signal**\n${analysis.researchOverlap}`,
    `\n*Analysis confidence: ${analysis.confidence} (based on data completeness)*`,
    "*Generated by Claude Haiku using structured SEC and OpenAlex data above.*\n",
  ].join("\n");
}
...
function sourceManifest(
  ...
  const tenKDesc = tenk?.business ?? '';
  const overviewFromTenK = tenKDesc.length >= 150;
  const rows: { section: string; source: string; status: string }[] = [
    {
      section: 'Company Overview',
      source: overviewFromTenK ? 'SEC 10-K Item 1' : 'Wikipedia',
      status: overviewFromTenK ? '✓ verified' : (wiki?.extract ? '⚠ fallback' : '✗ empty'),
    },
    {
      section: 'Key Risks',
      source: 'SEC 10-K Item 1A',
      status: (tenk?.risks?.length ?? 0) > 200 ? '✓ verified' : '✗ empty',
    },
    {
      section: 'Financial Data',
      source: 'SEC XBRL',
      status: (fin?.revenue?.length ?? 0) > 0 ? '✓ verified' : '✗ empty',
    },
    {
      section: 'MD&A / Outlook',
      source: 'SEC 10-K Item 7',
      status: (tenk?.mda?.length ?? 0) > 100 ? '✓ verified' : '✗ empty',
    },
    {
      section: 'Research Signals',
      source: 'OpenAlex',
      status: (research?.count ?? 0) > 0 ? '✓ verified' : '✗ empty',
    },
  ];
  const verified = rows.filter(r => r.status.startsWith('✓')).length;
  const table = [
    "| Section | Source | Status |",
    "|---|---|---|",
    ...rows.map(r => `| ${r.section} | ${r.source} | ${r.status} |`),
  ].join("\n");
  return `\n---\n<details>\n<summary>📊 Source Verification — ${verified}/${rows.length} sections verified from primary sources</summary>\n\n${table}\n</details>\n`;
}
