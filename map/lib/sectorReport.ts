...
export function buildSectorReport(report: any): SectorReportModel {
  ...
  // Priority matrix — sorted NIH → PubMed → Trial → None.
  const rank = (p: any) => uncPisOf(p).length ? 3 : p.existing_unc_tie ? 2 : (p.pipeline?.length ? 1 : 0);
  const matrix: MatrixRow[] = [...profiles].sort((a, b) => rank(b) - rank(a)).map((p) => {
    const pis = uncPisOf(p);
    const tier: "Strategic" | "Translational" = p.partnership_type === "Strategic" ? "Strategic" : "Translational";
    if (pis.length) {
      const pi = pis[0];
      return { company: p.company_name, tier, signal: "NIH grant", contact: pi.name, contactUrl: pi.grant_url, grantOrPmid: pi.grant_num || grantIdFromUrl(pi.grant_url || ""), grantUrl: pi.grant_url, firstMove: "Email PI · OSP first" };
    }
    if (p.existing_unc_tie) {
      const a = pubAlign(p)[0];
      const yr = a ? fy4(a.unc_fact || a.rationale || "") : "";
      return { company: p.company_name, tier, signal: "PubMed", contact: a ? (a.unc_unit || "Co-author") : "Co-author", contactUrl: a ? firstSource(a.sources) : undefined, grantOrPmid: undefined, grantUrl: a ? firstSource(a.sources) : undefined, firstMove: yr ? `Cite ${yr} paper` : "Cite paper" };
    }
    if (p.pipeline?.length) {
      return { company: p.company_name, tier, signal: "Trial", contact: "—", firstMove: "Reference trial" };
    }
    return { company: p.company_name, tier, signal: "None", contact: "—", firstMove: "Cold outreach" };
  });
  ...
  return {
    ...
    revenuePeers, rdPeers, matrix, faculty: faculty.slice(0, 12), dataAssets: DATA_ASSETS,
  };
}
