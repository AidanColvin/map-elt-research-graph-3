...
export interface DataAsset {
  name: string;
  url: string;
  description: string; // what it is — one concrete clause
  relevance: string;   // why a partner in this sector should care — the argument
  heldBy: string;
}
...
// Core UNC health data assets (the broad-health fallback uses the first three).
const DATA_ASSETS: DataAsset[] = [
  { name: "Carolina Data Warehouse for Health", url: "https://tracs.unc.edu/index.php/services/informatics-and-data-science", description: "De-identified EHR, labs, and imaging from the UNC Health system.", relevance: "Test clinical hypotheses on a large, real-world patient population.", heldBy: "NC TraCS / UNC Health" },
  { name: "NC AHEC Network Data", url: "https://www.ncahec.net/", description: "Health data spanning rural and underserved communities statewide.", relevance: "Study access, equity, and outcomes beyond major metros.", heldBy: "NC AHEC Program" },
  { name: "Lineberger Cancer Registry", url: "https://unclineberger.org/", description: "Linked cancer diagnosis, treatment, and outcomes data with imaging.", relevance: "Ground oncology work in real treatment pathways.", heldBy: "UNC Lineberger" },
  { name: "Biospecimen Processing Facility", url: "https://www.med.unc.edu/", description: "Banked tissue, blood, and biofluid samples for translational work.", relevance: "Source human samples for assay and biomarker validation.", heldBy: "UNC School of Medicine" },
  { name: "Sheps Center Rural Health Data", url: "https://www.shepscenter.unc.edu/", description: "Health-services and workforce data focused on rural NC.", relevance: "Model care delivery and workforce gaps.", heldBy: "Cecil G. Sheps Center" },
  { name: "NC TraCS Institute", url: "https://tracs.unc.edu/", description: "NIH CTSA hub with IRB and clinical-research coordination.", relevance: "Run regulated studies on built-in research infrastructure.", heldBy: "NC TraCS" },
];

// Sector-tailored UNC data assets. Each report surfaces the three assets most
// relevant to its topic, with a plain-language argument for why a partner in
// that sector should care. Sectors that match no group fall back to UNC's
// cross-cutting data-science infrastructure — never health datasets.
const ASSET_GROUPS: { match: RegExp; assets: DataAsset[] }[] = [
  { match: /oncolog|cancer|tumou?r|carcinoma|leukem|lymphoma|melanoma/i, assets: [
    { name: "Lineberger Cancer Registry & Tissue Procurement", url: "https://unclineberger.org/", description: "Linked treatment and outcomes data plus banked tumor biospecimens.", relevance: "Validate oncology targets and biomarkers on real patient cohorts.", heldBy: "UNC Lineberger" },
    { name: "High-Throughput Sequencing Facility", url: "https://www.med.unc.edu/", description: "Genomic and transcriptomic sequencing at production scale.", relevance: "Generate molecular evidence for drug and diagnostic programs.", heldBy: "UNC School of Medicine" },
    { name: "Carolina Data Warehouse for Health", url: "https://tracs.unc.edu/", description: "De-identified EHR, labs, and imaging across UNC Health.", relevance: "Run real-world studies on a large patient population.", heldBy: "NC TraCS / UNC Health" },
  ] },
  ...
  { match: /climate|energy|environment|sustainab|clean ?tech|carbon|coastal|marine|ocean|\bwater\b|utilit|\bpower\b|electric|\bgrid\b|nuclear|renewable|\bsolar\b|\bwind\b/i, assets: [
    { name: "UNC Institute for the Environment", url: "https://ie.unc.edu/", description: "Environmental modeling, energy-transition, and clean-tech research.", relevance: "Stress-test decarbonization plans and sustainability claims.", heldBy: "UNC-Chapel Hill" },
...
const GENERIC_ASSETS: DataAsset[] = [
  { name: "RENCI — Renaissance Computing Institute", url: "https://renci.org/", description: "Applied-AI engineering, cyberinfrastructure, and high-performance computing.", relevance: "Co-build data and ML systems at scale.", heldBy: "UNC-Chapel Hill" },
  { name: "School of Data Science and Society", url: "https://datascience.unc.edu/", description: "University-wide data-science faculty and graduate talent.", relevance: "Find research partners and hire data scientists.", heldBy: "UNC-Chapel Hill" },
