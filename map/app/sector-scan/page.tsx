  ...
  return (
    ...
    >
      {/* When embedded in the Map dashboard the global header already shows the
          brand, so the internal one only appears once a report's view nav exists. */}
      {(!embedded || status === 'done') && (
      <header
        style={{
          ...styles.header,
          ...(status === 'done' ? styles.headerSticky : {}),
          // Inside the dashboard the fixed global header is 52px tall — stick
          // the view nav just below it instead of underneath it.
          ...(status === 'done' && embedded ? { top: 52 } : {}),
        }}
      >
        {!embedded && (
        <button
