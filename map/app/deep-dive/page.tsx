...
import MarkdownArticle, { slugify } from "@/app/components/MarkdownArticle";
...
export default function Home({ embedded = false }: { embedded?: boolean }) {
  ...
  const [intro, setIntro] = useState(!embedded);
  const articleRef = useRef<HTMLDivElement>(null);

  // play the intro once per session; respect reduced-motion.
  // skipped entirely when embedded in the Map dashboard.
  useEffect(() => {
    if (embedded) return;
    ...
    return (
      <main className="hero">
        ...
        <div className="hero-inner">
          {/* The dashboard's segmented control already names the tool, so the
              big standalone title only renders on the standalone route. */}
          {!embedded && (
            <>
  ...
  return (
    <main className="app" style={{ ["--accent" as any]: accent }}>
      {intro && <IntroSplash onDone={() => setIntro(false)} />}
      {/* Inside the dashboard the fixed global header is 52px tall — stick
          the report topbar just below it instead of underneath it. */}
      <header className="topbar" style={embedded ? { top: 52 } : undefined}>
